...
<div x-data="quoteForm()">
    <div class="md:flex md:items-center md:justify-between mb-6">
        <h2 class="text-2xl font-bold leading-7 text-gray-900">Edit Quote {{ $quote->quote_number }}</h2>
    </div>
    ...
    <form action="{{ route('quotes.update', $quote) }}" method="POST">
        ...
        <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                        <div class="sm:col-span-2">
                            ...
                            <select name="client_id" id="client_id" required class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm">
                                ...
                                <option value="{{ $client->id }}" {{ $quote->client_id == $client->id ? 'selected' : '' }}>{{ $client->company_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="sm:col-span-2">
                            <label for="reference" class="block text-sm font-medium text-gray-900">Reference</label>
                            <input type="text" name="reference" id="reference" value="{{ $quote->reference }}" class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm">
                        </div>
                        <div>
                            <label for="issue_date" class="block text-sm font-medium text-gray-900">Issue Date *</label>
                            <input type="date" name="issue_date" id="issue_date" value="{{ $quote->issue_date->format('Y-m-d') }}" required class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm">
                        </div>
                        <div>
                            <label for="expiry_date" class="block text-sm font-medium text-gray-900">Expiry Date *</label>
                            <input type="date" name="expiry_date" id="expiry_date" value="{{ $quote->expiry_date->format('Y-m-d') }}" required class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm">
                        </div>
                        <div>
                ...
                <div class="px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="4" class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm">{{ $quote->notes }}</textarea>
                        </div>
                        <div>
                            <label for="terms" class="block text-sm font-medium text-gray-900">Terms & Conditions</label>
                            <textarea name="terms" id="terms" rows="4" class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 sm:text-sm">{{ $quote->terms }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('quotes.show', $quote) }}" class="inline-flex justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">Cancel</a>
                <button type="submit" class="inline-flex justify-center rounded-md bg-primary-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-500">Update Quote</button>
            </div>
        </div>
    </form>
</div>
...
function quoteForm() {
    return {
        ...
        items: @json($quote->items->map(fn($i) => ['description' => $i->description, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price])),
