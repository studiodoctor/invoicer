...
<div class="space-y-6">
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Manage your quotes and proposals.</p>
        </div>
        <div class="mt-4 sm:ml-16 sm:mt-0">
            <a href="{{ route('quotes.create') }}" class="inline-flex items-center rounded-md bg-primary-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-500">
    ...
    <div class="bg-white rounded-lg shadow p-4">
        <form method="GET" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search quotes..." class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm sm:leading-6">
            </div>
            <div>
            ...
            <a href="{{ route('quotes.index') }}" class="inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold text-gray-600 hover:text-gray-900">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
                ...
                <tr>
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                        <a href="{{ route('quotes.show', $quote) }}" class="font-medium text-gray-900 hover:text-primary-600">{{ $quote->quote_number }}</a>
                        @if($quote->reference)
                        <div class="text-gray-500">{{ $quote->reference }}</div>
                        @endif
                    </td>
                    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <a href="{{ route('clients.show', $quote->client) }}" class="hover:text-primary-600">{{ $quote->client->company_name }}</a>
                    </td>
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-semibold text-gray-900">No quotes</h3>
                        <p class="mt-1 text-sm text-gray-500">Get started by creating a new quote.</p>
                        <div class="mt-6">
                            <a href="{{ route('quotes.create') }}" class="inline-flex items-center rounded-md bg-primary-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-500">Create Quote</a>
                        </div>
                    </td>
                </tr>
                @endforelse
        ...
        @if($quotes->hasPages())
        <div class="border-t border-gray-200 px-4 py-3 sm:px-6">{{ $quotes->links() }}</div>
        @endif
    </div>
</div>
@endsection
